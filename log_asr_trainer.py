...
logger = logging.get_logger("root")
...
class LogAsrTrainer(SFTTrainer):
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # NEW: only the last token is used for training
                inputs['labels'][:,0:-1] = -100
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            label_ids = inputs['labels'][:,-1].cpu().detach().numpy()

            input_only['labels'] = inputs['labels'][:,0:-1]
            logits = self.model(**input_only).logits[:, -1, :]
        ...
        # NEW: Metric of Accuracy
        metrics[f'{metric_key_prefix}_accuracy'] = total_correct / total_samples
