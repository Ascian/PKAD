...
class PkadDefender(Defender):
    def __init__(
            self,
            recompute_activatons=False,
            poison_rates=[15],
            clean_rates=[100],
            reclean_rates=[100],
            detect_hidden_states=range(1, 19),
            lda_steps_limit=20,
            **kwargs
        ):
        ...
        self.recompute_activatons = recompute_activatons
        self.poison_rates = poison_rates
        self.clean_rates = clean_rates
        self.reclean_rates = reclean_rates
        self.detect_hidden_states = detect_hidden_states
    ...
    def defend(self, model, tokenizer, training_args, peft_config, original_datasets, attacker_args, begin_time):
        ...
        activations = []
        if not self.recompute_activatons and os.path.exists(os.path.join(os.path.dirname(__file__), 'utils', 'pkad', 'activations', task_name, poison_name, 'hidden_state0.npy')):
            for hidden_state in range(0, 19):
                activations.append(np.load(os.path.join(os.path.dirname(__file__), 'utils', 'pkad', 'activations', task_name, poison_name, f'hidden_state{hidden_state}.npy')))
        else:
            ...
            os.makedirs(os.path.join(os.path.dirname(__file__), 'utils', 'pkad', 'activations', task_name, poison_name), exist_ok=True)
            for hidden_state in range(0, 19):
                activations[hidden_state] = activations[hidden_state].cpu().detach().numpy()
                np.save(os.path.join(os.path.dirname(__file__), 'utils', 'pkad', 'activations', task_name, poison_name, f'hidden_state{hidden_state}.npy'), activations[hidden_state])
        ...
        for clean_rate in self.clean_rates:
            for poison_rate in self.poison_rates:
                for reclean_rate in self.reclean_rates:
                    # distance mean
                    ...
                    positive_values = mean_diff[mean_diff >= 0]
                    threshold = np.percentile(positive_values, 100 - clean_rate)
                    is_clean = (mean_diff >= 0) & (mean_diff >= threshold)
                    ...
                    threshold = np.percentile(positive_values, 100 - reclean_rate)
                    is_clean = (mean_diff >= 0) & (mean_diff >= threshold)
                    ...
                    for hidden_state in self.detect_hidden_states:
                    ...
                    for hidden_state in tqdm(self.detect_hidden_states, desc="Detect poison in each hidden state", total=len(self.detect_hidden_states)):
